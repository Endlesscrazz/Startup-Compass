...
const SECTOR_TO_INDUSTRY: Record<string, string> = {
  ...
  "Bio/Medical Tech": "Life Sciences and Healthcare",
  Consumer: "Consumer Packaged Goods",
  Energy: "Manufacturing",
  Marketplaces: "Software and Information Technology",
...
export function getAtlasStats() {
  ...
  const industries = new Set(RESOURCES.flatMap((resource) => resource.industries));
...
export function getDatasetSourceLabels() {
  return [
    "Map Data for Builder Day",
    "Resources List - Builder Day",
...
export function getEmployeeFilterValue() {
  return getEmployees().find((item) => item.value !== "Unknown")?.value ?? "Unknown";
}
...
export function getSignalOptions() {
  ...
  return [
    ...
    [
      "Has Description",
      true,
      formatCompactCount(COMPANIES.filter((company) => company.description).length),
    ],
    ["Complete Profiles", false, formatCompactCount(completeCount)],
  ] as [string, boolean, string][];
}

function stageTopics(stage: string) {
  const value = stage.toLowerCase();
  if (value.includes("pre") || value.includes("seed")) {
    return ["Start a Business", "Funding", "Entrepreneurship Communities"];
  }
  if (value.includes("series") || value.includes("growth")) {
    return ["Funding", "Late Stage Growth", "Marketing and Sales"];
  }
  if (value.includes("bootstrap")) {
    return ["Start a Business", "Taxes and Finance", "Marketing and Sales"];
  }
  return ["Entrepreneurship Communities", "Funding"];
}

export function getResourceRecommendations(
  company: Company = getFeaturedCompany(),
  limit = 3,
): Recommendation[] {
  const targetIndustry = SECTOR_TO_INDUSTRY[company.sector] ?? company.sector;
  const desiredTopics = stageTopics(company.stage);

  return RESOURCES.map((resource) => {
    const reasons: string[] = [];
    let score = 0;

    if (resource.industries.includes(targetIndustry)) {
      score += 44;
      reasons.push(`${targetIndustry} industry match`);
    }

    const matchedTopics = desiredTopics.filter((topic) =>
      resource.topics.includes(topic),
    );
    if (matchedTopics.length > 0) {
      score += matchedTopics.length * 16;
      reasons.push(`${matchedTopics.join(" / ")} topic match`);
    }

    if (resource.locations.length === 0 || resource.locations.length >= 20) {
      score += 8;
      reasons.push("statewide availability");
    }

    if (resource.communities.includes("Any")) {
      score += 5;
      reasons.push("open to any founder community");
    }

    if (resource.link) score += 3;
    ...
    return {
      ...
      fitLabel: score >= 80 ? "Excellent Fit" : score >= 58 ? "Strong Fit" : "Relevant Fit",
      tags,
      reasons: reasons.length > 0 ? reasons : ["available in the provided resource dataset"],
    };
  })
    .sort((a, b) => {
...
export function getCompanyFacts(company: Company) {
  return [
    ...
    ["Profile Type", company.displayType ?? "profile"],
